...
const Einbuchen = () => {
  ...
  // 1) Beim Laden: Buchungen aus sessionStorage holen (falls vorhanden)
  useEffect(() => {
    const stored = sessionStorage.getItem("einbuchenBookings");
    if (stored) {
      setBookings(JSON.parse(stored));
    }
  }, []);

  // 2) Immer wenn bookings sich ändern, in sessionStorage speichern
  useEffect(() => {
    sessionStorage.setItem("einbuchenBookings", JSON.stringify(bookings));
  }, [bookings]);
  ...
  return (
    <div className="text-white space-y-6">
      <h1 className="text-3xl font-bold mb-4">Einbuchen</h1>

      <div className="bg-gray-800 p-4 rounded shadow max-w-xl space-y-4">
        <h2 className="text-xl font-semibold">Neuen Wareneingang verbuchen</h2>
      ...
      <div className="space-y-2">
        <h2 className="text-xl font-semibold">Letzte Einbuchungen (Session only)</h2>
